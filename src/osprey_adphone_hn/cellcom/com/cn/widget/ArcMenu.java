...
public class ArcMenu extends ViewGroup implements OnClickListener {
	...
	public ArcMenu(Context context, AttributeSet attrs, int defStyle) {
		...
		mRadius = ta.getDimensionPixelOffset(R.styleable.ArcMenu_radius,
				(int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
						100, getResources().getDisplayMetrics()));

		System.out.println("position = " + mPosition + " , radius =  "
				+ mRadius);
		ta.recycle();
	}
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if (changed) {
			...
			// double angle = Math.acos((b1 * b1 + b1 * b1 - c1 * c1) / (2 * b1
			// * b1));
			...
			for (int i = 0; i < count - 1; i++) {
				...
				// int cl = (int) (mRadius * Math.sin(anglel * (i + 1) + a1 *
				// i));
				// int ct = (int) (mRadius * Math.cos(anglet * (i + 1) + a2 *
				// i));
		...
		for (int i = 0; i < count - 1; i++) {
			...
			if (!tag.equals(child.getTag())) {
				...
				if (mCButton != null && child != null) {
					width = (mCButton.getMeasuredWidth() - child
							.getMeasuredWidth()) / 2;
					height = (mCButton.getMeasuredHeight() - child
							.getMeasuredHeight()) / 2;
				}
				if (mCurrentStatus == Status.CLOSE) {
					ta = new TranslateAnimation(xflag * (cl - width), 0, yflag
							* (ct - height), 0);
					child.setClickable(true);
					child.setFocusable(true);
				} else { // to close
					ta = new TranslateAnimation(0, xflag * (cl - width), 0,
							yflag * (ct - height));
	...
	private Animation scaleSmallAnim(int duration) {
		AnimationSet as = new AnimationSet(true);
		ScaleAnimation sa = new ScaleAnimation(1.0f, 0, 1.0f, 0,
	...
	private Animation scaleBigAnim(int duration) {
		AnimationSet as = new AnimationSet(true);
		ScaleAnimation sa = new ScaleAnimation(1.0f, 4.0f, 1.0f, 4.0f,
				Animation.RELATIVE_TO_SELF, 0.5f, Animation.RELATIVE_TO_SELF,
				0.5f);
		AlphaAnimation aa = new AlphaAnimation(1.0f, 0);
		as.addAnimation(sa);
