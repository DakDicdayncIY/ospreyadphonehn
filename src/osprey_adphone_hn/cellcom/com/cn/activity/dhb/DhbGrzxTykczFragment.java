...
public class DhbGrzxTykczFragment extends FragmentBase {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		// TODO Auto-generated method stub
		View view = inflater.inflate(R.layout.os_grzx_tykczfragment, container,
				false);
		initView(view);
		return view;
	}
		...
		HttpHelper.getInstances(act).send(FlowConsts.YYW_SAO_GETWIN,
				...
				new CellComHttpInterface.NetCallBack<CellComAjaxResult>() {
					...
					public void onSuccess(CellComAjaxResult cellComAjaxResult) {
						...
						if ("Y".equalsIgnoreCase(sysPrizeComm.getBody()
								.getIfwin())) {
	 ...
	 */
	private void getNextAd(final String ggid, final GrzxCz grzxCz) {
		if (SharepreferenceUtil.readString(act, SharepreferenceUtil.fileName,
				"uid", "").equals("")) {
			Intent loginintent = new Intent(act, LoginActivity.class);
			startActivity(loginintent);
		...
		HttpHelper.getInstances(act).send(FlowConsts.YYW_GETLOOKGGINFO,
				cellComAjaxParams, CellComAjaxHttp.HttpWayMode.POST,
				new CellComHttpInterface.NetCallBack<CellComAjaxResult>() {
					...
					public void onSuccess(CellComAjaxResult cellComAjaxResult) {
						...
						if (!FlowConsts.STATUE_1.equals(kykAdComm
								.getReturnCode())) {
							Toast.makeText(act, kykAdComm.getReturnMessage(),
									Toast.LENGTH_SHORT).show();
							return;
						}

						KykAdResult kykadresult = kykAdComm.getBody();
						Intent intent = new Intent(act, SysAdShowActivity.class);
